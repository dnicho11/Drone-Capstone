...
const int ESC_PIN    = 5;
const int CELL_DT    = 3;
const int CELL_SCK   = 4;

Servo servo;
HX711 cell(CELL_DT, CELL_SCK);
...
void setup() {
  ...
  servo.attach(ESC_PIN); // servo signal pin
  servo.writeMicroseconds(1000);
  
  ads.setGain(GAIN_ONE);
  ads.begin();
  
  attachInterrupt(digitalPinToInterrupt(RPM_PIN), rpmCounterInterrupt, FALLING); //laser rx on pin
